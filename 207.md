...
**Definition**: A set of vectors $\{\vector{u_{1}}, \dots, \vector{u_{n}}\}$ is called **linearly dependent** if there exists a solution to the equation above where $a_{1}, a_{2}, a_{3}, \dots, a_{n}$ aren't all zero.
...
**Theorem**: Suppose that the Wrońskian $W(f_{1}(x), \dots, f_{n}(x))$ is nonzero on at least one value in an interval $I$. Then, the functions $f_{1}(x), \dots, f_{n}(x)$ are linearly independent over the interval $I$.
