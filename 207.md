...
**Example**: $T: M_{2}(\R) \rightarrow M_{2}(\R), T\left(\begin{bmatrix}a & b\\ c & d\end{bmatrix}\right) = \begin{bmatrix}d & c\\ b & a+1\end{bmatrix}$
...
Check 1) We proved $T(\vec{0}_{V}) = \vec{0}_{W}$, so $\vec{0}\in ker(T)$.

Check 2) Take $u,v\in ker(T)$. So, $T(\vec{u})=\vec{0}$ and $T(\vec{v})=\vec{0}$. Then,
...
Check 3) Take $\vec{u}\in ker(T), c\in\R$. So, $T(\vec{u})=\vec{0}$. Then,
