...
**Definition**: An **ordinary differential equation** is a differential equation which relates a function of a single-variable and its derivatives. The most general form of a differential equation is
\begin{equation*}
F(x,y,y',y'',y''',\dots,y^{(n-1)},y^{(n)}) = 0
\end{equation*}
where $n$ is called the **order** of the differential equation.

**Definition**: An **integral curve** ...
...
**Definition**: A **separable equation** is an equation of the form
\begin{equation*}
y' = f(x)g(y).
\end{equation*}
...
**Definition**: An **autonomous equation** is an equation of the form
\begin{equation*}
\frac{\d y}{\d t} = g(y).
\end{equation*}
...
**Definition**: A **homogeneous equation** is an equation of the form
\begin{equation*}
...
\begin{equation*}
\psi_{x}(x,y) = M(x,y)\quad\text{and}\quad\psi_{y}(x,y) = N(x,y).
\end{equation*}

### Bernoulli Equations

**Definition**: A **Bernoulli equation** is an equation of the form
\begin{equation*}
y' + p(x)y = q(x)y^{n}.
\end{equation*}
...
6) $\forall\vector{u}\in V, \forall c\in\R$ if $c\vector{u} = \vector{0}$, then $\vector{u} = \vector{0}$, $c = 0$, or both.
...
Subspaces of $\mathbb{R}^{2}$ are either a point on the origin (trivial subspace), a line through the origin, or the entire space.

Subspaces of $\mathbb{R}^{3}$ are either a point on the origin, a line through the origin, a plane through the origin, or the entire space.

### Spanning Sets

#### Linear Combinations

**Definition**: A **linear combination** of a set of vectors $\vector{u_{1}}, \vector{u_{2}}, \vector{u_{3}}, \dots, \vector{u_{n}}$ is of the form
\begin{equation*}
a_{1}\vector{u_{1}} + a_{2}\vector{u_{2}} + a_{3}\vector{u_{3}} + \dots + a_{n}\vector{u_{n}}
\end{equation*}
where $a_{1}, a_{2}, a_{3}, \dots, a_{n}\in\R$.

**Definition**: The set of all such linear combinations is called the **span** of $\{\vector{u_{1}}, \dots, \vector{u_{n}}\}$ and is denoted by
\begin{equation*}
\text{span}\{\vector{u_{1}}, \dots, \vector{u_{n}}\} = \{a_{1}\vector{u_{1}} + a_{2}\vector{u_{2}} + a_{3}\vector{u_{3}} + \dots + a_{n}\vector{u_{n}}:a_{1}, \dots, a_{n}\in\R\}.
\end{equation*}

**Theorem**: The span of a set of vectors in $V$ is a subspace of $V$.

**Definition**: If $V = \text{span}\{\vector{u_{1}}, \dots, \vector{u_{n}}\}$, $\{\vector{u_{1}}, \dots, \vector{u_{n}}\}$ is a spanning set for $V$.

### Linear Independence

Consider the equation $a_{1}\vector{u_{1}} + a_{2}\vector{u_{2}} + a_{3}\vector{u_{3}} + \dots + a_{n}\vector{u_{n}} = \vector{0}$.

**Definition**: A set of vectors $\{\vector{u_{1}}, \dots, \vector{u_{n}}\}$ is called **linearly independent** if the only solution to the equation above is $a_{1} = 0, a_{2} = 0, a_{3} = 0, \dots, a_{n} = 0$ which is also known as the **trivial solution**.

**Definition**: A set of vectors $\{\vector{u_{1}}, \dots, \vector{u_{n}}\}$ is called **linearly dependent** if there exists a solution to the equation above where $a_{1}, a_{2}, a_{3}, \dots, a_{n}$ aren't all zero, in other words, a **non-trivial solution**.

**Theorem**: Suppose $\{\vector{u_{1}}, \dots, \vector{u_{n}}\}$ is a linearly dependent set of vectors and $\vector{u_{n}}$ can be written as a linear combination of $\{\vector{u_{1}}, \dots, \vector{u_{n-1}}\}$. Then,
\begin{equation*}
\text{span}\{\vector{u_{1}}, \dots, \vector{u_{n}}\} = \text{span}\{\vector{u_{1}}, \dots, \vector{u_{n-1}}\}.
\end{equation*}

The equation $a_{1}\vector{u_{1}} + a_{2}\vector{u_{2}} + a_{3}\vector{u_{3}} + \dots + a_{n}\vector{u_{n}} = \vector{0}$ is equivalent to the system of linear homogeneous equations $A\vector{a} = \vector{0}$ where $A$ is the $m \times n$ matrix whose column vectors are $\vector{u_{1}}, \dots, \vector{u_{n}}$ and $\vector{a}$ is the vector whose entries are $a_{1}, \dots, a_{n}$.

**Theorem**: Let $A$ be the $m \times n$ matrix whose column vectors are $\vector{u_{1}}, \dots, \vector{u_{n}}$. Then, $\{\vector{u_{1}}, \dots, \vector{u_{n}}\}$ is linearly independent if and only if the only solution to $A\vector{a} = \vector{0}$ is the trivial solution $\vector{a} = \vector{0}$. Equivalently, this is true if and only if the system has no free variables.

#### The Wrońskian

**Definition**: Let $f_{1}(x), \dots, f_{n}(x)$ be a set of $n$ functions which are all differentiable at least $n-1$ times. The **Wrońskian** of these functions is the determinant expression
\begin{equation*}
W(f_{1}(x), \dots, f_{n}(x)) = \text{det}\left[\begin{array}{cccc}f_{1}(x) & f_{2}(x) & \dots & f_{n}(x)\\ f_{1}'(x) & f_{2}'(x) & \dots & f_{n}'(x)\\ \vdots & \vdots & \ddots & \vdots\\ f_{1}^{(n-1)}(x) & f_{2}^{(n-1)}(x) & \dots & f_{n}^{(n-1)}(x)\end{array}\right].
\end{equation*}

**Theorem**: Suppose that the Wrońskian $W(f_{1}(x), \dots, f_{n}(x))$ is non-zero on at least one value in an interval $I$. Then, the functions $f_{1}(x), \dots, f_{n}(x)$ are linearly independent over the interval $I$.

Note that in the theorem above, this is not an if and only if statement. It is possible for the Wrońskian to be zero and the set of functions to be linearly independent.

#### Vandermonde Determinant

**Definition**: Suppose that $c_{1},\dots,c_{n}$ are real or complex numbers. The **Vandermonde matrix** for these numbers is
\begin{equation*}
V(c_{1},\dots,c_{n}) = \left[\begin{array}{cccc}1 & 1 & \dots & 1\\ c_{1} & c_{2} & \dots & c_{n}\\ \vdots & \vdots & \ddots & \vdots\\ c_{1}^{n-1} & c_{2}^{n-1} & \dots & c_{n}^{n-1}\end{array}\right].
\end{equation*}
...
**Corollary**: If $\text{dim}(V) = n$, then any set of $n$ spanning vectors is a basis.
...
**Corollary**: If $\text{dim}(V) = n$, then any set of $n$ linearly independent vectors is a basis.
...
Take $f(x)\in C^{n}(I)$. Then, $f'(x), f''(x), f'''(x), \dots, f^{n}(x)$ all exist and are continuous on $I$. Therefore, $f'(x)$ exists and $f'(x)\in C^{n-1}(I)$ is equal to $f'(x)\in C^{n-1}(I)$ and is well-defined.
...
\begin{equation*}
A = \begin{bmatrix}T(\vector{e}_{1}) & T(\vector{e}_{2}) & T(\vector{e}_{3}) & \dots & T(\vector{e}_{n})\end{bmatrix}
\end{equation*}
where $\vector{e}_{1}, \vector{e}_{2}, \vector{e}_{3}, \dots, \vector{e}_{n}$ are the standard basis vectors of $\R^{n}$.
...
**Definition**: A $\mathbf n^{\textbf{th}}$**-order linear ODE** is an equation of the form
\begin{equation*}
a_{n}(x)y^{(n)} + a_{n-1}(x)y^{(n-1)} + a_{n-2}(x)y^{(n-2)} + \dots + a_{1}(x)y' + a_{0}(x)y = f(x)
\end{equation*}

where $a_{n}(x), a_{n-1}(x), a_{n-2}(x), \dots, a_{1}(x), a_{0}(x)$ are known as coefficient functions and $f(x)$ is known as the forcing function.
...
An initial condition for a $n^{\text{th}}$-order ODE is of the form
...
Consider a $n^{\text{th}}$-order linear initial value problem

\begin{equation*}
a_{n}(x)y^{(n)} + a_{n-1}(x)y^{(n-1)} + a_{n-2}(x)y^{(n-2)} + \dots + a_{1}(x)y' + a_{0}(x)y = f(x)
\end{equation*}

with $y(x_{0}) = y_{0}, y'(x_{0}) = y_{1}, y''(x_{0}) = y_{2}, \dots, y^{(n-1)}(x_{0}) = y_{n-1}$.

Observe that if
- $I$ is an interval containing $x_{0}$
- $a_{0}(x), \dots, a_{n}(x), f(x)$ are continuous on $I$
- $a_{n}(x) \neq 0$ on $I$
...
**Example**: $y''' - y' = 0, y(0) = 8, y'(0) = 3, y''(0) = 0$
...
Therefore, our general solution is
\begin{align*}
y &= C_{1}\e^{-1t} + C_{2}\e^{0t} + C_{3}\e^{1t}\\
  &= C_{1}\e^{-t} + C_{2} + C_{3}\e^{t}.
\end{align*}
Using $y(0) = 8$ we have,
\begin{align*}
8 &= C_{1}\e^{0} + C_{2} + C_{3}\e^{0}\\
  &= C_{1} + C_{2} + C_{3}.
\end{align*}
Taking the derivative of $y$ we have,
\begin{equation*}
...
3 &= -C_{1}\e^{0} + C_{3}\e^{0}\\
  &= -C_{1} + C_{3}.
\end{align*}
Taking the second derivative of $y$ we have,
\begin{equation*}
y'' = C_{1}\e^{-t} + C_{3}\e^{t}.
\end{equation*}
Using $y''(0) = 0$ we have,
...
\begin{align*}
8 &= C_{1} + C_{2} + C_{3}\\
  &= -\frac{3}{2} + C_{2} + \frac{3}{2}\\
  &= C_{2}.
\end{align*}
Therefore, our solution to the initial value problem is
\begin{equation*}
y = -\frac{3}{2}\e^{-t} + 8 + \frac{3}{2}\e^{t}.
\end{equation*}

**Definition**: A linear ODE is **regular** on $I$ if

- $a_{0}(x), \dots, a_{n}(x), f(x)$ are continuous on $I$
- $a_{n}(x) \neq 0$ for all $x\in I$.

**Theorem**: The set of all solutions to a homogeneous $n^{\text{th}}$-order linear ODE which is regular on $I$ is a $n$-dimensional subspace of $C^{n}(I)$.

**Corollary**: The general solution of a $n^{\text{th}}$-order differential equation should have $n$ constants.

**Theorem**: If $y = f_{1}(x), y = f_{2}(x), y = f_{3}(x), \dots, y = f_{m}(x)$ are all solutions to the linear differential equation
\begin{equation*}
a_{n}(x)y^{(n)} + a_{n-1}(x)y^{(n-1)} + a_{n-2}(x)y^{(n-2)} + \dots + a_{1}(x)y' + a_{0}(x)y = f(x)
\end{equation*}
which is regular on $I$, then for some $x\in I$, $W(f_{1}(x),\dots,f_{m}(x)) \neq 0$ if and only if $f_{1}(x),\dots,f_{m}(x)$ are linearly independent in $C^{n}(I)$.
...
\end{equation*}
and the **quadratic formula** for finding roots
\begin{equation*}
...
### Second-Order Method of Variation of Parameters

### Third-Order Method of Variation of Parameters
